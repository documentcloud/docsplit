  ...
  class ImageExtractor
    ...
    def convert(pdf, size, format, previous=nil)
      ...
      if previous
        ...
        result = `MAGICK_TMPDIR=#{tempdir} OMP_NUM_THREADS=2 #{"gm" if TOOLCHAIN == "graphicsmagick"} mogrify #{common} -unsharp 0x0.5+0.75 \"#{directory}/*.#{format}\" 2>&1`.chomp
        raise ExtractionFailed, result if $? != 0
      else
        page_list(pages).each do |page|
          out_file  = ESCAPE[File.join(directory, "#{basename}_#{page}.#{format}")]
          cmd = "MAGICK_TMPDIR=#{tempdir} OMP_NUM_THREADS=2 #{"gm" if TOOLCHAIN == "graphicsmagick"} convert +adjoin -define pdf:use-cropbox=true #{common} #{escaped_pdf}[#{page - 1}] #{out_file} 2>&1".chomp
          result = `#{cmd}`.chomp
          raise ExtractionFailed, result if $? != 0
        end
      end
